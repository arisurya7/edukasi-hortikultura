...
@section('content')
    <div class="container">
         ...
         <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">Tips Tanaman</h1>
        </div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        @endif
                        <form method="POST" action="{{ isset($data) ? route('plant-tips.update', $data->id) :  route('plant-tips.store')}}" id="formDataPlant" enctype="multipart/form-data">
                            @csrf
                            @if (isset($data))
                                {{ method_field('PUT') }}
                            @endif
                            <div class="mb-3">
                                <label for="title" class="form-label">Judul Tips Tanaman<span class="text-danger">*</span>
                                </label>
                                <input type="text" name="name" class="form-control" id="name" title="name"
                                    placeholder="masukan judul tips tanaman" value="{{ $data->name ?? '' }}" required>
                            </div>
                            ...
                            <a href="{{ route('plant-tips.index') }}" class="btn btn-outline-primary" id="back_button">Back</a>
                        </form>
                    </div>
                ...
                Swal.fire({
                    ...
                    if (data.status) {
                        Swal.fire(data.message, '', 'success').then(function() {
                            window.location = "{{ route('plant-tips.index') }}";
                        });
                    }
                });
